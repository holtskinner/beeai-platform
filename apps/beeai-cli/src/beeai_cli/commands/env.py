...
import httpx
import typer
from beeai_sdk.platform import ModelProvider, ModelProviderType, SystemConfiguration, Variables
...
LLM_PROVIDERS = [
    Choice(
        ...
        value=(ModelProviderType.anthropic, "Anthropic Claude", "https://api.anthropic.com/v1", "claude-sonnet-4-0"),
    ),
    Choice(
    ...
    Choice(
        ...
        value=(ModelProviderType.name, "NVIDIA NIM", "https://integrate.api.nvidia.com/v1", "deepseek-ai/deepseek-r1"),
    ),
    Choice(
    ...
    Choice(
        ...
        value=(ModelProviderType.other, ModelProviderType.other, None, None),
    ),
]
...
EMBEDDING_PROVIDERS = [
    ...
    Choice(
        name="Mistral".ljust(25) + "🆓 has a free tier",
        value=(ModelProviderType.mistral, "https://api.mistral.ai/v1", "mistral-embed"),
    ),
    Choice(
        name=ModelProviderType.ollama.ljust(25) + "💻 local",
        value=(ModelProviderType.ollama, "http://localhost:11434/v1", "nomic-embed-text:latest"),
    ),
    Choice(
        name="OpenAI".ljust(25),
        value=(ModelProviderType.openai, "https://api.openai.com/v1", "text-embedding-3-small"),
    ),
    Choice(
...
async def _configure_llm() -> dict[str, str | None] | None:
    ...
    selected_model: str
    provider_type, provider_name, api_base, recommended_model = await inquirer.fuzzy(  # type: ignore
    ...
    return {
        ...
        "name": provider_name,
        "type": provider_type,
        "default_llm_model": selected_model,
        **extra_config,
    }
...
async def _configure_embedding(env: dict[str, str | None]) -> dict[str, str | None] | None:
    ...
    return {
        ...
        "name": provider_name,
        "type": provider_type,
        "default_embedding_model": selected_model,
        **extra_config,
    }
...
async def setup(
    ...
    with verbosity(verbose):
        ...
        if not use_true_localhost:
            llm_env["api_base"] = re.sub(r"localhost|127\.0\.0\.1", "host.docker.internal", llm_env["api_base"] or "")
            if embedding_env:
                embedding_env["api_base"] = re.sub(
                    r"localhost|127\.0\.0\.1", "host.docker.internal", embedding_env["api_base"] or ""
                )

        with console.status("Saving configuration...", spinner="dots"):
            async with configuration.use_platform_client():
                await ModelProvider.create(**llm_env)  # pyright: ignore [reportArgumentType]

                if llm_env["api_base"] != embedding_env["api_base"]:
                    await ModelProvider.create(**embedding_env)  # pyright: ignore [reportArgumentType]
